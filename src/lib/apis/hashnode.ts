...
const QUERIES = {
    ...
    SUBSCRIBE_TO_NEWSLETTER: gql`
        mutation subscribeToNewsletter($publicationId: ObjectId!, $email: String!) {
            subscribeToNewsletter(input: { email: $email, publicationId: $publicationId }) {
                status
            }
        }
    `,
...
class BlogAPIError extends Error {
    constructor(
        ...
        super(originalError instanceof Error ? `${message}: ${originalError.message}` : message);
...
async function executeGraphQLRequest<T>(
    ...
    try {
        return await request<T>(env.NEXT_PUBLIC_HASHNODE_GQL_ENDPOINT, query, variables);
    } catch (error) {
...
export async function getBlogPostByID(id: string): Promise<{ post: IBlogByIDResponse['post'] }> {
...
export async function getBlogPostIDBySlug(slug: string): Promise<{ id: string } | null> {
    ...
    try {
        ...
        // Üçüncü yöntemi dene
        const directResponse = await executeGraphQLRequest<IBlogPostIDBySlugResponse>(
...
export async function getAllBlogPostsSlug(
    ...
    const { pageSize: normalizedPageSize } = normalizePaginationParams(pageSize, page);
...
export async function getBlogPostsCardMeta({
    ...
    const { pageSize: normalizedPageSize, page: normalizedPage } = normalizePaginationParams(
        pageSize,
        page
    );
...
export async function subscribeToNewsletter(email: string): Promise<{ status: string }> {
