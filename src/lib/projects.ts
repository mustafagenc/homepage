...
function validateDirectory(dir: string) {
    try {
        ...
        throw new ProjectError(`Directory ${dir} does not exist or is not accessible`, error);
    }
}
...
function getMDXFiles({ dir }: { dir: string }): string[] {
    ...
    try {
        ...
        throw new ProjectError(`Failed to read MDX files from directory: ${dir}`, error);
    }
}
...
export function getProjectByTitle({ title }: { title: string }): IProject | null {
    ...
    try {
        ...
        throw new ProjectError(`Error reading project file: ${projectFilePath}`, error);
    }
}
...
export function getProjectMetadata({
    ...
    try {
        ...
        throw new ProjectError(`Error reading metadata for file: ${projectAbsFilePath}`, error);
    }
}
        ...
        const projectsWithMetadata = projectFiles
            ...
            .filter(
                (metadata): metadata is IProjectMetadata =>
                    metadata !== null && metadata.topics?.includes(PROJECT_FILTER_TOPIC) === true
            )
