...
import { FilterFormSchema, TFilterFormSchema } from '@/lib/validators/filter-form';
...
export const FilterDropdown = ({ endpoint, defaultPerPage }: FilterDropdownProps) => {
    ...
    const perPageSearchParamRaw = searchParams?.get(PER_PAGE_QUERY_PARAM) ?? null;
    const pageSearchParamRaw = searchParams?.get(PAGE_QUERY_PARAM) ?? null;
    ...
    const handleFormSubmit: SubmitHandler<TFilterFormSchema> = async (data: TFilterFormSchema) => {
    ...
    return (
        <div className="flex gap-2">
            <Popover open={isOpen} onOpenChange={setIsOpen}>
                <PopoverTrigger asChild onClick={() => setIsOpen((prev) => !prev)} className="mb-4">
                    <Button
                        variant="outline"
                ...
                <PopoverContent className="w-64 p-4" onFocusOutside={() => setIsOpen(false)}>
                    <Form {...form}>
